        ...
        <CardContent>
          ...
          <div className="grid grid-cols-3 text-center font-semibold">
            ...
            <div>
              ...
              <p className={totalRemaining < 0 ? "text-red-600" : "text-green-600"}>₹{totalRemaining}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardContent>
          <table className="w-full border">
            ...
            <tbody>
              {report.map((item, i) => {
                const percent = item.budget ? Math.min((item.spent / item.budget) * 100, 100) : 0;
                return (
                  <tr key={i} className="text-center">
                    <td className="p-2 border">{item.category}</td>
                    <td className="p-2 border">₹{item.budget}</td>
                    <td className="p-2 border">₹{item.spent}</td>
                    <td className={`p-2 border font-semibold ${item.remaining < 0 ? "text-red-600" : "text-green-600"}`}>
                      ₹{item.remaining}
                    </td>
